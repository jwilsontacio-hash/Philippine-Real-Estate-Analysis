...
def geocode_addresses(df_pre_cleaned, gmaps_client, location_column="Location", rate_limit=0.02):
    def extract_components(messy_address):
        if pd.isna(messy_address) or not str(messy_address).strip():
            return None, None, None, "Blank Input"  # Changed to None
        ...
        try:
            ...
            if not geocode_result:
                return None, None, None, "No Match Found"  # Changed to None
            ...
            barangay = city_municipality = province = None  # Changed to None
            google_region = None  # Changed to None
            ...
            if google_region == "National Capital Region" or "Metro Manila" in formatted_address:
                if province is None or province == "Not Found":  # Updated condition
            ...
            if barangay in (city_municipality, province):
                barangay = None  # Changed to None
            
            return barangay, city_municipality, province, formatted_address
        except Exception:
            return None, None, None, "Error"  # Changed to None
    ...
    for idx, row in tqdm(df_pre_cleaned.iterrows(), total=len(df_pre_cleaned), desc="Geocoding Progress"):
        ...
        row["Town/Barangay"] = brgy  # Renamed from "Barangay"
        row["Municipality/City"] = city  # Renamed from "City_Municipality"
        row["Province/NCR"] = prov  # Renamed from "Province"
...
- From the information given, and clues parse them. Look at the available data or information or 
knowledge that you know about to which barangay, municipality or city, province the Location Address 
and its Google Cleaned Address belongs
- Keep the ñ of the names. 
- Make sure to keep the consistent standard formal naming of places. 
If it is given in native Filipino name e.g. Kalakhang Maynila, Encode "Metro Manila";
 'Lalawigan ng Laguna' encode Laguna; 'Lalawigan ng Cebu' encode Cebu.; 'Makati' encode as Makati City
- If you cannot find any information, write "Not Found"
...
def LLM_treatment(data_frame, client):
    ...
    # Initialize renamed columns if missing
    for col in ['Town/Barangay', 'Municipality/City', 'Province/NCR']:
        if col not in df.columns:
            df[col] = None
    
    # Check for null values in renamed columns
